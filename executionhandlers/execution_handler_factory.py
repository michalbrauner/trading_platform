...
class ExecutionHandlerFactory:
    ...
    @staticmethod
    def create_from_settings(configuration: Configuration, data_handler: DataHandler, events: queue.Queue,
        ...
        if configuration.execution_handler_name == OandaExecutionHandler:
            return ExecutionHandlerFactory.create_oanda_execution_handler(data_handler, events, events_per_symbol,
    ...
    @staticmethod
    def create_historic_csv_execution_handler(data_handler, events: queue.Queue,
                                              events_per_symbol: Dict[str, queue.Queue]) -> ExecutionHandler:
        return SimulatedExecutionHandler(data_handler, events, events_per_symbol)

    @staticmethod
    def create_oanda_execution_handler(data_handler: DataHandler, events: queue.Queue,
                                       events_per_symbol: Dict[str, queue.Queue], account_id: str, access_token: str,
                                       logger: Logger) -> ExecutionHandler:
        return OandaExecutionHandler(data_handler, events, events_per_symbol, account_id, access_token, logger)
